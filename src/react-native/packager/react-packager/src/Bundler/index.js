...
class Bundler {
  ...
  bundle(options) {
    ...
    const moduleSystemDeps =
      this._resolver.getModuleSystemDependencies({dev, unbundle, includeFramework});  // @Denis
    return this._bundle({
    ...
    return Promise.resolve(resolutionResponse).then(response => {
      ...
      if (response.dependencies.length > 1) { // skip HMR requests
        const numModuleSystemDependencies =
          this._resolver.getModuleSystemDependencies({dev, unbundle, includeFramework}).length; // @Denis
  ...
  getDependencies({
    ...
    return this.getTransformOptions(
      ...
      return this._resolver.getDependencies(
        entryFile,
        {dev, platform, recursive, includeFramework}, // @Denis
        transformOptions,
        onProgress,
      );
    });
